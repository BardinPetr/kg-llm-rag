...
from ededup.ededup import EDedupService
from utils.graphutils import g_nodes_of_type
from kg.kgextract import EntityKG, FactKG
from db.neo_kg import KType, KFactType, KEntity, KFact, KRelFact, KValFact
...
def fact_ingest(d_block: DBlock):
    ...
    for k, v in nodes.items():
        ...
        if isinstance(d_block, DTxtBlock):
            proof = dict(
                overview=str(d_block.own_context[max(0, v.ref_pos - 100):v.ref_pos+100] + "..."),
                loc_begin=v.ref_pos,
                loc_type="char"
            )
